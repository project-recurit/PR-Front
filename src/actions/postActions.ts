...
/** 게시물 상세 요청 액션 */
export const getPostDetail = handleError(async (postType: MainCategory, postId: string): Promise<PJPost | PRPost> => {
  const res = await fetch(`${POST_API_URL[postType]}/${postId}`, {
    method: "GET",
    headers: commonHeaders,
    cache: "no-store",
  });
